...
| `playwright.yml`         | `pull_request` → `main` / manual                                                                         | Waits for the Cloudflare Pages preview deploy via `scripts/ci/wait-for-cf-preview.sh`, then runs Playwright against the preview URL. `workflow_dispatch` takes a `base_url` input instead. On failure it uploads **both** `playwright-report/` (the HTML report) and `test-results/` (per-test screenshots, traces, `error-context.md`) as the `playwright-report` artifact. Those two directories must stay distinct: the HTML reporter clears its `outputFolder` before writing, so pointing it at `test-results/` silently deletes the run's own failure evidence (#220).                                                                                                                                                                                                                                                                                                                                                              |
| `refresh-test-stats.yml` | `push` → `main` (`paths: tests/**`, `**/*.test.ts`, `playwright.config.ts`) / manual                     | Runs `pnpm stats:refresh` to regenerate `src/lib/testStats.ts` (the `/testing` unit/E2E counts) whenever specs change, then commits it back to `main` via the ephemeral GitHub App token (`vars.CI_BOT_APP_ID` + `secrets.CI_BOT_PRIVATE_KEY`, a ruleset bypass actor — no standing PAT, #201) if it changed. The commit touches only `testStats.ts`, which matches neither this workflow's `paths` filter (no self-trigger) nor version-bump's `paths-ignore` (no bump no-op).                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `version-bump.yml`       | `push` → `main` (`paths-ignore: ci-snapshot.json`, `project-stats.json`, `testStats.ts`, `public/og/**`) | Reads the merged commits via Conventional Commits — `feat`→minor, `fix`/`perf`→patch, `feat!`/`BREAKING CHANGE`→major, anything else→no bump — then `npm version <level> --no-git-tag-version` and commits the bump back to `main` via the ephemeral GitHub App token (`vars.CI_BOT_APP_ID` + `secrets.CI_BOT_PRIVATE_KEY`, a ruleset bypass actor — no standing PAT, #201). The bump commit deliberately omits `[skip ci]` (Cloudflare Pages honors it, which would stop the footer version from ever deploying); its `chore(release)` subject re-runs this workflow once as a no-op rather than looping. The `paths-ignore` keeps the generated-file refreshes (ci-snapshot, project-stats, test stats, OG images) from triggering a bump.                                                                                                                                                                                              |
