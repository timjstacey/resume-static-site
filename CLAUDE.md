...
| `publish-linkedin.yml`      | `push` → `main` (`paths: src/content/posts/**`)                                          | Blog-first publish. When a new blog post lands on `main`, reads its `linkedinPost` frontmatter and `repository_dispatch`es a `linkedin-publish` event (`client_payload: { linkedin_text, blog_url, slug }`) to the `linkedin-post-generator` repo, which posts the copy to LinkedIn (the blog link is already in it). Posts without `linkedinPost` (hand-written) are skipped. One-way link, no write-back. Uses `secrets.LPG_DISPATCH_TOKEN`.                                                                                                                                           |
| `notify-linkedin.yml`       | `push` → `main` (`paths: src/content/posts/**`)                                          | **Legacy (LinkedIn-first), removed in #159.** When a cross-posted blog post lands on `main`, reads its `linkedinUrl` + `linkedinComment` frontmatter and `repository_dispatch`es a `blog-published` event to `linkedin-post-generator` to comment the link back on the original LinkedIn post. Superseded by `publish-linkedin.yml`; posts now carry `linkedinPost`, not these fields, so it no-ops until torn down.                                                                                                                                                                     |
