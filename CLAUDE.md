...
| `notify-linkedin.yml`       | `push` → `main` (`paths: src/content/posts/**`)                                          | When a cross-posted blog post lands on `main`, reads its `linkedinUrl` + `linkedinComment` frontmatter and `repository_dispatch`es a `blog-published` event (with the live blog URL) to the `linkedin-post-generator` repo, which comments the link back on the original LinkedIn post. Posts without a `linkedinUrl` (hand-written) are skipped. Uses `secrets.LPG_DISPATCH_TOKEN`.                                                                                                                                                                                                     |
