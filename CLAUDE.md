...
src/
  ...
  lib/
    ...
    testStats.ts        # Generated test counts surfaced on /testing — `pnpm stats:refresh` (also run nightly by refresh-ci-snapshot.yml)
...
| `refresh-ci-snapshot.yml` | nightly cron (`0 6 * * *`) / manual                         | Runs `pnpm ci:refresh` (regenerates `src/data/ci-snapshot.json` from the live Actions API) and `pnpm stats:refresh` (regenerates `src/lib/testStats.ts` from the spec inventory), then opens a single PR with the bumps if anything changed. Keeps the footer, the `/testing` CI strip, and the test counts from going stale.                                                                                                                                                                                                                      |
